...
static Json state(Model model) {
    ...
    for (const auto& [id, w] : model.windows)
        out["windows"].push_back(Json::array({id, w.app, w.focused, w.seen, w.mru,
            w.workspace ? Json(*w.workspace) : Json(),
    ...
    for (bool current : {false, true}) {
        for (const auto& g : model.groups(current)) {
            ...
            out[current ? "current" : "all"].push_back(Json::array({g.key, g.app, g.windows,
                g.active, target ? Json(*target) : Json()}));
        }
    }
    out["next_seen"] = model.parse(Json::parse(
        R"({"id":18446744073709551615,"app_id":"probe","is_focused":false})")).seen;
    return out;
}
static void incremental_events() {
    Model m;
    auto feed = [&](const char* event) { assert(m.apply(Json::parse(event))); };
    feed(R"({"WindowsChanged":{"windows":[{"id":1,"app_id":"a","is_focused":true,"workspace_id":10,"layout":{"pos_in_scrolling_layout":[1,1]}},{"id":2,"app_id":"b","is_focused":false,"workspace_id":20,"layout":{"pos_in_scrolling_layout":[2,1]}}]}})");
    feed(R"({"WorkspacesChanged":{"workspaces":[{"id":10,"idx":2,"output":"DP-1","is_focused":true},{"id":20,"idx":1,"output":"DP-1"}]}})");
    auto reject = [&](const Json& event) {
        const auto before = state(m);
        bool threw = false;
        try { m.apply(event); } catch (const std::exception&) { threw = true; }
        assert(threw);
        assert(state(m) == before);
    };
    for (const char* event : {
        R"({"WorkspaceActivated":{"id":20}})",
        R"({"WorkspaceActivated":{"id":20,"focused":1}})",
        R"({"WorkspaceActivated":{"id":-1,"focused":false}})",
        R"({"WindowClosed":{}})", R"({"WindowClosed":{"id":-1}})",
        R"({"WindowClosed":{"id":"1"}})",
        R"({"WindowFocusChanged":{}})", R"({"WindowFocusChanged":{"id":-1}})",
        R"({"WindowFocusChanged":{"id":true}})",
    ...
    oversized["WindowLayoutsChanged"]["changes"] = std::vector<Json>(16385, Json::array({Id(1), Json::object()}));
    reject(oversized);
    ...
    assert(state(m) == before);
    feed(R"({"WindowLayoutsChanged":{"changes":[[1,{"pos_in_scrolling_layout":[8,1]}],[2,{}],[1,{"pos_in_scrolling_layout":[4,2]}],[999,{}]]}})");
    ...
    feed(R"({"WindowOpenedOrChanged":{"window":{"id":2,"app_id":"b","is_focused":false,"workspace_id":20}},"WindowClosed":{}})");
    assert(state(m) == before);
}
int main() {
    incremental_events();
