    ...
    bool pump(int64_t now, bool stream, const std::function<void(const Json &)> &line) {
        ...
        while (lines < 128) {
            ...
            if (end != std::string::npos) {
                auto payload = input.substr(0, end);
                input.erase(0, end + 1);
                ...
                try {
                    ...
                    if (j.contains("Err"))
                        throw std::runtime_error("niri rejected request");
            ...
            input.append(buffer, static_cast<size_t>(n));
    ...
    void tick(int64_t now) {
        ...
        if (events.open()) {
            bool ok = events.pump(now, true, [&](const Json &j) {
                if (model.apply(j) && j.contains("WindowsChanged"))
                    connected = true;
            });
            if (!ok) {
                connected = false;
        ...
        if (action.open())
            action.pump(now, false, [](const Json &j) {
                if (!j.contains("Ok"))
                    throw std::runtime_error("invalid action reply");
            });
        if (queued && !action.open() && connected) {
            Id id = *queued;
            queued.reset();
            if (model.is_current(id))
                action.start(path, Json{{"Action", {{"FocusWindow", {{"id", id}}}}}}.dump() + "\n",
                             now);
        }
    }
    void focus(Id id) {
