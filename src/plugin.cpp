...
int contribute(void* self, OtterPluginFrameCtx* frame, const OtterPluginSlotConstraints* cons) {
    if (!self || !cons) return -1;
    try {
        auto& s = *static_cast<Instance*>(self);
        s.client.tick(now_ms());
        if (s.displayed_workspace != s.client.model.focused_workspace) s.page = 0;
        ...
        if (a.height == 0 || capacity == 0) return 0;
        ...
        for (size_t i = 0; i < visible; ++i) {
            ...
            OtterPluginRect r{left + static_cast<int32_t>(i * item_width), a.y, item_width, a.height};
            // Hidden portions must not leave clickable targets behind.
            if (!contains(cons->clip, r)) continue;
            uint64_t hit = s.next_hit++;
            if (s.next_hit == more_hit) s.next_hit = 1;
            s.hits.emplace(hit, g.key);
            auto node = "app-" + g.key;
            auto icon = icons->resolve(g.app);
            int rc = s.host->sd_bar_item(frame, node.c_str(), &r, "", 0, icon.c_str(),
            ...
            if (g.windows.size() > 1 && r.height >= 14 && s.host->sd_rect && s.host->sd_label) {
                ...
                auto count = g.windows.size() > 9 ? std::string("9+") : std::to_string(g.windows.size());
                auto bg = node + "-count-bg", label = node + "-count";
                rc = s.host->sd_rect(frame, bg.c_str(), &badge, OTTER_PLUGIN_COLOR_THEME_BACKGROUND_ALT);
                if (rc) return rc;
                rc = s.host->sd_label(frame, label.c_str(), &badge, count.c_str(), 10, OTTER_PLUGIN_COLOR_THEME_FOREGROUND);
                if (rc) return rc;
        ...
        if (overflow) {
            OtterPluginRect r{left + static_cast<int32_t>(visible * item_width), a.y, item_width, a.height};
        ...
        return 0;
    } catch (...) { return -1; } // No C++ exception crosses the C ABI.
}
void destroy(void* self) { delete static_cast<Instance*>(self); }
int event(void* self, const OtterPluginEvent* e) {
    if (!self || !e) return -1;
    if (e->kind != OTTER_PLUGIN_EVENT_CLICK) return 0;
    try {
        auto& s = *static_cast<Instance*>(self);
        auto it = s.hits.find(e->hit_data);
        if (it == s.hits.end()) return 0;
        s.client.tick(now_ms());
        ...
        if (s.displayed_workspace != s.client.model.focused_workspace) {
            if (s.host->request_frame) s.host->request_frame(s.host->userdata);
            return 0;
        }
        if (e->hit_data == more_hit) ++s.page;
        else {
            std::string key = it->second;
            if (auto id = s.client.model.target(key, true)) s.client.focus(*id);
            s.client.tick(now_ms());
...
extern "C" {
__attribute__((visibility("default"))) uint32_t otter_plugin_abi_version() { return OTTER_PLUGIN_ABI_VERSION; }
__attribute__((visibility("default"))) int otter_plugin_init(const OtterPluginHost* host) {
    if (!host || host->abi_version != OTTER_PLUGIN_ABI_VERSION) return -1;
    ...
    if (!host->sd_bar_item) return -1;
    try {
        if (!icons) icons = std::make_unique<Icons>(Icons::roots());
        auto value = env("OTTER_RUNNING_APPS_SLOTS", "8");
        char* end = nullptr; auto n = std::strtoul(value.c_str(), &end, 10);
        slots = end && *end == '\0' && n >= 2 && n <= 32 ? static_cast<unsigned>(n) : 8;
        initialized = true;
        return 0;
    } catch (...) { return -1; }
}
__attribute__((visibility("default"))) void otter_plugin_deinit() { initialized = false; icons.reset(); }
...
__attribute__((visibility("default"))) OtterPluginBarInstance* otter_plugin_bar_attach(const OtterPluginHost* host, const char*) {
    if (!initialized || !host || host->abi_version != 1 || !host->sd_bar_item) return nullptr;
    try {
