...
struct Window {
    ...
    std::string app;
    bool focused;
    uint64_t seen = 0, mru = 0;
    std::optional<Id> workspace = {};
    std::optional<std::pair<Id, Id>> position = {};
};
struct Group {
    std::string key, app;
...
class Model {
    ...
    static std::optional<std::pair<Id, Id>> position(const Json &layout) {
        if (!layout.contains("pos_in_scrolling_layout") ||
            layout.at("pos_in_scrolling_layout").is_null())
            return {};
        const auto &p = layout.at("pos_in_scrolling_layout");
        if (!p.is_array() || p.size() != 2)
            throw std::runtime_error("invalid layout position");
    ...
    auto location(const Window &w) const {
        ...
        return std::make_tuple(it == workspaces.end() ? std::string("~") : it->second.output,
                               it == workspaces.end() ? w.workspace.value_or(unknown)
                                                      : it->second.index,
                               w.workspace.value_or(unknown), !w.position.has_value(),
                               w.position ? w.position->first : unknown,
                               w.position ? w.position->second : unknown, w.seen, w.id);
    }
  ...
  public:
    ...
    bool is_current(Id id) const {
        ...
        return focused_workspace && it != windows.end() &&
               it->second.workspace == focused_workspace;
    }
    void clear() {
        windows.clear();
        workspaces.clear();
        focused_workspace.reset();
    }
    ...
    Window parse(const Json &j) {
        ...
        if (!j.at("app_id").is_null())
            w.app = j.at("app_id").get<std::string>();
        ...
        if (j.contains("workspace_id") && !j.at("workspace_id").is_null())
            w.workspace = id_value(j.at("workspace_id"));
        if (j.contains("layout"))
            w.position = position(j.at("layout"));
        return w;
    }
    ...
    bool apply(const Json &e) {
        ...
        if (!e.contains("WorkspacesChanged") && !e.contains("WindowsChanged") &&
            ...
            if (e.contains("WindowClosed")) {
                ...
                return false; // Future niri events and extra fields are harmless.
            return true;
        }
        Model next = *this;
        if (e.contains("WorkspacesChanged")) {
            const auto &list = e.at("WorkspacesChanged").at("workspaces");
            if (!list.is_array() || list.size() > 16384)
                throw std::runtime_error("invalid workspaces");
            next.workspaces.clear();
            next.focused_workspace.reset();
            for (const auto &j : list) {
                auto output =
                    j.at("output").is_null() ? std::string("~") : j.at("output").get<std::string>();
                Id id = id_value(j.at("id"));
                next.workspaces.emplace(id, Workspace{output, id_value(j.at("idx"))});
                if (j.value("is_focused", false))
                    next.focused_workspace = id;
            }
            ...
            for (const auto &j : list) {
                ...
                if (w.focused)
                    active = w.id;
                if (!replacement.emplace(w.id, w).second)
            ...
            if (next.windows.size() >= 16384 && !next.windows.count(w.id))
                throw std::runtime_error("too many windows");
            next.windows[w.id] = w;
            if (w.focused)
                next.focus(w.id);
        }
    ...
    std::vector<Group> groups(bool current_only = false) const {
        ...
        for (auto &[k, g] : by_key) {
            std::sort(g.windows.begin(), g.windows.end(), [&](Id a, Id b) {
                return location(windows.at(a)) < location(windows.at(b));
            });
            out.push_back(std::move(g));
        }
        std::sort(out.begin(), out.end(), [&](const auto &a, const auto &b) {
            return location(windows.at(a.windows.front())) <
                   location(windows.at(b.windows.front()));
        });
        return out;
    }
    std::optional<Id> target(const std::string &group_key, bool current_only = false) const {
        for (const auto &g : groups(current_only))
            if (g.key == group_key) {
                // An active group cycles in layout order; an inactive group restores MRU.
                for (size_t i = 0; i < g.windows.size(); ++i)
                    if (windows.at(g.windows[i]).focused)
                        return g.windows[(i + 1) % g.windows.size()];
