...
inline std::string env(const char* name, std::string fallback = "") {
    const char* v = std::getenv(name); return v && *v ? v : fallback;
}
...
class Icons {
    ...
    static std::string folded(std::string value) {
        for (char& c : value) if (static_cast<unsigned char>(c) < 128) c = static_cast<char>(std::tolower(static_cast<unsigned char>(c)));
        return value;
    }
public:
    explicit Icons(const std::vector<std::string>& roots) {
        ...
        for (const auto& root : roots) {
            ...
            for (; !ec && it != end && count < 20000; it.increment(ec), ++count) {
                ...
                if (it->path().extension() == ".desktop" && it->is_regular_file(ec)) files.push_back(it->path());
            }
            ...
            for (const auto& file : files) {
                ...
                while (std::getline(in, line) && (bytes += line.size()) < 262144) {
                    ...
                    if (line[0] == '[') { section = line == "[Desktop Entry]"; continue; }
                    if (!section) continue;
                    auto pos = line.find('='); if (pos == std::string::npos) continue;
                    auto name = trim(line.substr(0, pos)), value = trim(line.substr(pos + 1));
                    if (name == "Icon") icon = value;
                    if (name == "StartupWMClass") wmclass = value;
                    if (name == "Hidden") hidden = value == "true";
                    if (name == "Type") type = value;
                }
                // Keep the mask for Hidden=true even if the lower-priority entry exists.
                if (hidden || type != "Application") { exact.emplace(id, "sparkles"); continue; }
                // ABI promises themed names only. Absolute files need a host image API.
                if (icon.empty() || icon.find('/') != std::string::npos || icon.size() > 4096 || icon.find('\0') != std::string::npos)
                    icon = "sparkles";
                exact.emplace(id, icon);
                if (!wmclass.empty()) aliases.emplace(wmclass, icon);
            }
        }
        // Precompute once; missing icon IDs must not scan all desktop entries every frame.
        for (const auto* entries : {&exact, &aliases}) for (const auto& [name, icon] : *entries) {
            auto [it, added] = folded_icons.emplace(folded(name), icon);
            if (!added && it->second != icon) it->second.clear();
        }
    }
    std::string resolve(std::string app) const {
        if (app.size() > 8 && app.compare(app.size() - 8, 8, ".desktop") == 0) app.resize(app.size() - 8);
        if (auto it = exact.find(app); it != exact.end()) return it->second;
        if (auto it = aliases.find(app); it != aliases.end()) return it->second;
        ...
        if (auto it = folded_icons.find(folded(app)); it != folded_icons.end() && !it->second.empty()) return it->second;
    ...
    static std::vector<std::string> roots() {
        ...
        std::string part;
        while (std::getline(dirs, part, ':')) if (!part.empty() && part[0] == '/') out.push_back(part);
